...
class CommandMessage:
    class CommandType(Enum):
        ...
        @classmethod
        def resolve_message_type(cls):
            """
            进行用户选择的消息类型的获取
            :return: 消息的类型
            """
            answers_for_message_type = PyInquirer.prompt(qm.COMMAND_TYPE_QUESTION)["command_type"]
            if answers_for_message_type == "normal":
                return cls.NORMAL_COMMAND
            elif answers_for_message_type == "length":
                return cls.LENGTH_COMMAND
            elif answers_for_message_type == "bloom":
                return cls.BLOOM_COMMAND
            else:
                raise ValueError("unsupported message type")
    ...
    def __str__(self):
        ...
        :return: 数据包对应的字符串
        """
        return f"type:{CommandMessage.CommandType(self.message_type)} | length:{self.length} | payload:{str(self.payload, encoding='utf-8')}"


if __name__ == "__main__":
    normal_message = CommandMessage(message_type=CommandMessage.CommandType.NORMAL_COMMAND, payload="hello world")
